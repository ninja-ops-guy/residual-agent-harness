...
Qualification-v1 remains independent testing-branch evidence. After reverse-merge **#333**, open #152 is now at exact testing-branch head **`11c0ac67f60f61a7243bcc79ce803d588a89aa94`**. Exact-head Qualification-v1 run `35443955204` attempt 1 is **FAIL** with required-gate failures in `deterministic-regression`, `qualification-selftests`, and `toxic-provider-matrix`. The deterministic gate retained seven enterprise sandbox failures because the hosted runner reported `kernel-level sandbox isolation unavailable`. A full provider-mission qualifier returned `FAIL`, causing the selftest and toxic-provider gates to fail; its lower-level cause remains **UNKNOWN** from the retained gate evidence. Factory ownership, M4 prerequisites, browser VM, Controller/provider, Command Station, clean install and other surrounding lanes are **PASS**, but do not override the overall required-gate FAIL. Protected maintainer approval and PR Agent advisory are also **FAIL**. The predecessor `24816ebc...` branch PASS remains historical exact-head evidence only.
