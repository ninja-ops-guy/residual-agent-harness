...
That is **implementation and reviewed mechanism closure, not blanket live qualification**. Merged `main` is not yet namespace-qualified by PR #109 because that PR is still unmerged. The protected lifecycle-test repair received the recorded owner-supplied independent technical acceptance, and the branch was refreshed onto current `main` without changing that reviewed blob or any other protected Factory byte. Current #109 head `4c44fe07f7e214c2c25ab8ff48f06e81ff5c86b9` advances only the reviewed lifecycle-test ownership pin; the other 37 protected pins, the provenance anchor and executable protected blobs remain unchanged. All applicable exact-head GitHub workflows are green. Synthetic merge `fea9429b2d111851fa074d6e72eb1892e3797d60`, tree `39599b2296fa8865ab1e0dabae333b9b6f933da8`, passed all 12 capability probes, actual `linux-userns-isolated-v1` execution, and 142 cases plus 84 subtests with zero skips in run `35032478689`. That qualifies the named PR candidate, not merged `main`, and does not complete release/recovery, soak, live-model or research gates.
...
Current `main` is `7cf45ea8eab675b870502fb76db1b6e63b562e9f`. It has advanced through the Mission Control real-provider reliability work in PR #122, the provenance-only Factory ownership anchor finalization in PR #121, PR #124's narrowly scoped immutable disk-chunk recovery, PR #125's completion-marker publication, and PR #127's guest-shell workbench dispatch. PRs #125 and #127 are WebVM/workbench changes; they do not alter Factory/M4 authority or evidence schemas. The exact-main push workflows, including Pages run `35031823399`, are green. These merges do not by themselves prove an acceptable long-run WebVM failure rate. See [`docs/CURRENT_STATUS.md`](docs/CURRENT_STATUS.md) for the exact CI state and active qualification blockers.

Before paper-facing live reliability evaluation, the priority gates are now:

1. complete merge review for PR #109 without broadening the already reviewed one-pin trust change; exact current-main head `4c44fe07...` is green across every applicable GitHub workflow, and M4 run `35032478689` passed all 12 capability probes, actual isolated execution, and 142 cases plus 84 subtests with zero skips, but this evidence remains bound to the PR candidate until merge and any required accepted-main confirmation;
