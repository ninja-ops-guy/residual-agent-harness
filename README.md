...
Two newer open candidates are not accepted current-main capability. **#338** changes protected `RuntimeJournal` bytes and advances the Factory ownership baseline to add bounded constructor write-admission retry; its exact-head Factory/technical lanes and maintainer-approval gate are PASS, while generated PR Pages and PR Agent advisory are FAIL. Because it touches protected bytes and the ownership baseline, it requires independent trust-boundary review and must not be auto-merged. **#340** proposes Moonshot/Kimi and Kimi Claw/OpenClaw provider/runtime adapters; it remains open with qualification still incomplete and maintainer approval FAIL, so it is only a branch candidate.
...
Qualification-v1 is also a separate branch evidence path. **#339** merged only into `testing/qualification-v1`, moving open PR **#152** to exact head `19d3917079ee6f7105e78c88e2aae08d25ce4c13`. Its first exact-head Qualification-v1 run, **`35446710781` attempt 1, remains FAIL**, but the failure narrowed materially: `qualification-selftests` is now **PASS**, the toxic-provider job is **PASS**, and the required deterministic regression gate remains **FAIL**. The deterministic gate failed at `Full deterministic regression gate`; the prior retained evidence identified the hosted-runner kernel-sandbox capability gap, and #339 deliberately did not weaken or skip that fail-closed requirement. #152 therefore remains unaccepted; protected maintainer approval and PR Agent advisory are still FAIL. The prior `11c0ac67...` run `35443955204` remains historical FAIL evidence for that exact predecessor head, including its provider-mission failures, and the earlier `24816ebc...` positive technical evidence remains exact-head historical evidence only. None of this branch evidence becomes accepted current-main capability or broadens Factory/M4 claims.
...
2. Diagnose the remaining **#152 deterministic Qualification-v1 FAIL** on changed head `19d391707...` without waiving the hosted-runner sandbox requirement. Preserve both the current run `35446710781` and predecessor run `35443955204` as revision-bound evidence.
3. Treat **#338** as a protected trust-boundary candidate only; do not auto-merge its RuntimeJournal/ownership-baseline pin change despite green scoped technical/approval lanes while Pages/advisory remain FAIL.
...
6. Validate the merged #320 policy on the production Vercel origin before calling response-header remediation PASS in production.
7. Retain a fresh real-account provider candidate→verifier→receipt success on an accepted deployed revision, or keep live-provider success `UNKNOWN`; #340 is not accepted evidence.
8. Complete physical mobile fallback validation, true blank-environment installation, recovery/host-loss qualification, selected elapsed soak, and #120/#126 long-run WebVM reliability work without broadening bounded evidence.
