...
async function main(){
  ...
  assert(await page.locator('#dialog-body').innerText().then(t=>t.includes('Acceptance checks')));assert((await page.locator('#dialog-body').innerText()).includes('Bound verification receipt'));checks.push('Task drawer exposes the station receipt and prerequisite binding');await page.screenshot({path:path.join(out,'04-task-evidence.png'),fullPage:true});await page.getByRole('button',{name:'Close dialog',exact:true}).click();checks.push('Task drawer shows checks and revision evidence');
  ...
  const dismissDraft=page.getByRole('button',{name:'Dismiss generated draft'});await dismissDraft.waitFor({timeout:7000});await dismissDraft.click();assert.equal(await page.getByRole('button',{name:/DRAFT SPEC.*Open generated draft/}).count(),0);await page.reload();assert.equal(await page.getByRole('button',{name:/DRAFT SPEC.*Open generated draft/}).count(),0);checks.push('Completed generated drafts can be dismissed persistently');
