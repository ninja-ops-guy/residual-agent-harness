...
class InspectorTests(unittest.TestCase):
    ...
    def test_unqualified_binary_is_never_executed(self):
        with patch("residual.inspector._run") as run:
            with self.assertRaises(ContractError):
                Px0Inspector(self.fake_px0, expected_binary_sha256="0" * 64)
        run.assert_not_called()

    def test_changed_binary_is_rejected_before_launch(self):
        inspector = self.inspector()
        self.fake_px0.write_text("#!/bin/sh\nexit 0\n")
        with patch("residual.inspector._spawn") as spawn:
            with self.assertRaises(ContractError):
                inspector.launch(self.root, open_browser=False, sandbox=False)
