...
from residual.eval_frozen import (
    CONFIGURATIONS,
...
from residual.eval_frozen.__main__ import run_fixture_study
from residual.eval_frozen.configs import constants_agree
from residual.eval_frozen.workload import SCHEMA_VERSION, workload_from_json
...
def test_r5_worker_correctness_invariant_to_control_layers(workload):
    ...
    from residual.eval_frozen.runner import _worker_correct
...
def test_acceptance_cli_runs_end_to_end(tmp_path):
    result = subprocess.run(
        [sys.executable, "-m", "residual.eval_frozen", "--out", str(tmp_path)],
...
def test_r9_report_hash_reproducible_across_processes(tmp_path):
    """Regression: two independent CLI regenerations must produce a
    byte-identical report hash (pinned report sha256 reproducibility)."""
    hashes = []
    for name in ("run1", "run2"):
        out = tmp_path / name
        result = subprocess.run(
            [sys.executable, "-m", "residual.eval_frozen", "--out", str(out)],
            capture_output=True, text=True, cwd=ROOT)
        assert result.returncode == 0, result.stderr
        summary = json.loads(result.stdout)
        assert summary["evidence_level"] == "development_fixture"
