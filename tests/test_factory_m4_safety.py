...
class M4AcceptanceSafetyTests(unittest.TestCase):
    ...
    def test_default_refuses_host_execution_before_worktree_creation(self):
        with patch.object(self.f.integrator, '_worktree') as worktree:
            with patch('residual.factory.m4_integrator.run_trusted_fixture') as runner:
                with self.assertRaisesRegex(m4.M4IntegrationError, 'untrusted project verification blocked'):
    ...
    def test_failed_tree_lookup_is_an_error_not_absence(self):
        ...
        def fail(repository, *args, **kwargs):
            if args[0] == 'ls-tree':
                raise WorkerContractError('controlled unavailable Git evidence')
            return real_git(repository, *args, **kwargs)
        with patch.object(m4, 'git', side_effect=fail), self.assertRaises(WorkerContractError):
