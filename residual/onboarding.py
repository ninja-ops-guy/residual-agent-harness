...
from .providers import HTTPProvider, NoRedirect, ProviderError
...
def _atomic_private_write(path: Path, text: str) -> None:
    ...
    # mkstemp creates a unique, exclusive 0600 inode before any secret bytes
    # are written; a predictable .tmp path could follow an attacker symlink.
...
def _stop_bootstrap_container(paths: ManagedPaths, clean_env: dict[str, str]) -> bool:
    """Report whether cleanup actually removed the credential-bearing service."""
    try:
        return _run(["docker", "compose", "down", "--remove-orphans"],
                    cwd=paths.service_dir, env=clean_env, timeout=60).returncode == 0
...
def start_service(paths: ManagedPaths, bootstrap: dict | None = None) -> tuple[bool, str]:
    ...
    # Explicit empty override prevents Compose falling back to stale .env data.
    clean["FREEAPI_CONFIG_JSON"] = ""
    env = clean.copy()
    if bootstrap:
    ...
    try:
        started = _run(["docker", "compose", "up", "-d"],
                       cwd=paths.service_dir, env=env).returncode == 0
        if started and bootstrap:
            imported_ready = wait_for_http("http://127.0.0.1:3001/", timeout=45)
    except (OSError, subprocess.TimeoutExpired):
        started = False
    finally:
        # Even a failed/timed-out compose invocation may have created a
        # container. Always scrub if bootstrap material was supplied.
        if bootstrap:
            scrubbed = False
            try:
                scrubbed = _run(
                ...
                pass
            if not scrubbed:
                stopped = _stop_bootstrap_container(paths, clean)
                detail = ("managed service stopped" if stopped else
                          "cleanup unconfirmed; remove the managed container before continuing")
                return False, "credential scrub failed; " + detail
    if not started:
        return False, "docker compose start failed"
    if bootstrap and not imported_ready:
...
def wait_for_http(url: str, timeout: int = 30, headers: dict | None = None) -> bool:
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            if exc.code < 500:
                return True
        except (urllib.error.URLError, TimeoutError, OSError, ProviderError):
            pass
        time.sleep(0.5)
    return False
...
def endpoint_ready(api_key: str | None = None) -> bool:
    ...
    except (urllib.error.HTTPError, urllib.error.URLError, TimeoutError, OSError, ProviderError):
        return False
