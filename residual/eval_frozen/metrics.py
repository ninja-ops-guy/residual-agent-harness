...
def _stable(value: float | None) -> float | None:
    """Normalize aggregate floats to a fixed precision (9 decimal places).

    Aggregate means/rates can otherwise differ in the last ulp across runs or
    platforms; rounding before serialization keeps the report byte-identical
    and its sha256 reproducible (EVAL-R9 determinism)."""
...
def compute_slice_metrics(config_id: str, slice_name: str,
    ...
    return SliceMetrics(
        ...
        P_X=_stable(len(correct) / n),
        P_A=_stable(len(accepted) / n),
        P_X_given_A=_stable(_rate(len(accepted_correct), len(accepted))),
        aer_far=_stable(_rate(len(accepted_incorrect), len(accepted))),
        isr=_stable(_rate(sum(1 for r in incorrect if not r.accepted), len(incorrect))),
        assr=_stable(len(accepted_correct) / n),
        acceptance_coverage=_stable(len(accepted) / n),
        false_rejection_rate=_stable(
            _rate(sum(1 for r in correct if not r.accepted), len(correct))),
        fcr=_stable(_rate(len(fault_caught), len(fault_labeled))) if fault_labeled else None,
        throughput_runs_per_sec=_stable(
            (n / (total_latency / 1000.0)) if total_latency else None),
        latency_ms_mean=_stable(total_latency / n),
        latency_ms_p50=_stable(statistics.median(r.latency_ms for r in records)),
        latency_ms_p95=_stable(_percentile([r.latency_ms for r in records], 0.95)),
        rework_per_run=_stable(sum(r.rework_attempts for r in records) / n),
        conflicts_total=sum(r.conflicts for r in records),
        conflict_rate=_stable(sum(r.conflicts for r in records) / n),
        verifier_rejection_rate=_stable(
            sum(1 for r in records if r.verifier_rejected) / n),
        input_tokens=total_input,
        output_tokens=total_output,
        cost_usd_total=_stable(total_cost),
        cost_per_accepted_correct_usd=_stable(
