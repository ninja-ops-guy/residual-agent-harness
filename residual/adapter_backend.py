...
SpecCommitVerifier = Callable[[str], bool]
...
def _resolve_spec_commit_in_repo(spec_repo: str, spec_head_sha: str) -> None:
    """Verify spec_head_sha resolves to an immutable ref tip in the spec repo.

    The normative contract forbids minting tokens against an untagged or
    uncommitted spec, so the commit must be reachable as a ref (e.g. a tag),
    not merely exist as a dangling object. spec_repo is host configuration and
    is passed to git as a fixed argv element, never through a shell.
    """
    try:
        out = subprocess.run(
            ["git", "ls-remote", spec_repo],
            check=True, capture_output=True, text=True, timeout=60,
        ).stdout
    except (OSError, subprocess.CalledProcessError, subprocess.TimeoutExpired) as exc:
        raise ContractError(
            f"spec repository {spec_repo!r} unreachable; cannot resolve spec_head_sha"
        ) from exc
    tips = {line.split()[0] for line in out.splitlines() if line.strip()}
    if spec_head_sha not in tips:
        raise ContractError(
            "spec_head_sha is not a ref tip in the configured spec repository; "
            "tag the spec commit or supply a spec_commit_verifier over a trusted "
            "immutable spec manifest"
        )
...
class LiveCoreResidualBackend:
    ...
    def __init__(
        ...
        *,
        ...
        spec_commit_verifier: SpecCommitVerifier | None = None,
        issuer: str = "residual-agent-harness/live-core",
    ):
        if not isinstance(spec_head_sha, str) or not _HEX40.fullmatch(spec_head_sha):
            raise ContractError("spec_head_sha must be a lowercase 40-hex git commit")
        # Fail closed on spec identity: an attestation may only be minted
        # against a spec commit that resolves to immutable repository state.
        if spec_commit_verifier is not None:
            try:
                verified = bool(spec_commit_verifier(spec_head_sha))
            except Exception as exc:
                raise ContractError("spec commit verification raised") from exc
            if not verified:
                raise ContractError(
                    "spec_head_sha did not resolve to a committed spec state"
                )
        elif spec_repo is not None:
            _resolve_spec_commit_in_repo(spec_repo, spec_head_sha)
        else:
            raise ContractError(
                "spec identity is unverified: supply spec_repo or a "
                "spec_commit_verifier over a trusted immutable spec manifest"
            )
        ...
        if not _HEX40.fullmatch(impl_commit_sha) or not _HEX40.fullmatch(impl_tree_sha):
            raise ContractError("implementation commit/tree must be lowercase 40-hex git ids")
        ...
        self._conn.commit()
        # One reentrant serialization boundary guards every lifecycle
        # transition: open/terminal state checks, event admission, and state
        # mutation are atomic per boundary, so a run cannot be terminalized
        # twice and no event can be admitted after terminalization.
    ...
    def _record_module_decision(
        ...
        *,
        ...
        # State check, event admission, and verdict transition are one atomic
        # boundary: a concurrent on_run_complete cannot interleave between the
        # open-run check and the module.called append.
        with self._lock:
    ...
    def on_run_complete(self, run_id: str, outcome: str) -> None:
        ...
        # Exactly-one-terminal is enforced atomically: the open/terminal checks,
        # the terminal event append, attestation admission, and the in-memory
        # transition all hold the serialization boundary. A second thread
        # completing the same run_id observes a terminal run and is rejected.
        with self._lock:
            if run_id not in self._started or run_id in self._terminal:
                raise LedgerWriteError(f"run not open: {run_id}")
