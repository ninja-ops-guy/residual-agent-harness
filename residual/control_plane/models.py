...
class IntentState(str, Enum): PREPARED="prepared"; ATTEMPTED="attempted"; APPLIED="applied"; FAILED="failed"; AMBIGUOUS="ambiguous"; RECONCILING="reconciling"; COMPENSATING="compensating"; COMPENSATED="compensated"
class TransactionState(str, Enum): PREPARED="prepared"; IN_PROGRESS="in_progress"; COMPLETE="complete"; INCOMPLETE="incomplete"; FAILED="failed"; RECONCILIATION_REQUIRED="reconciliation_required"; COMPENSATING="compensating"; COMPENSATED="compensated"
...
class Mission:
    mission_id: str; principal_id: str; tenant_id: str="default"; created_at: float=field(default_factory=time.time)
...
class MissionRevision:
    ...
    capability_grants: tuple[CapabilityGrant,...]=(); parent_revision: Optional[str]=None; amendment_id: Optional[str]=None
...
@dataclass(frozen=True)
class AmendmentDelta:
    delta_type: str; value: Any=None; authority_delta: Mapping[str, Any]=field(default_factory=dict)
    resource_delta: Mapping[str, Any]=field(default_factory=dict); risk_score: Optional[int]=None

@dataclass(frozen=True)
class PlanAmendment:
    amendment_id: str; mission_id: str; from_revision: str; amendment_class: AmendmentClass
    reason: str; proposed_delta: Mapping[str, Any]=field(default_factory=dict); deltas: tuple[AmendmentDelta,...]=()
    classifier_revision: str="amendment-classifier-v1"; counterexample_refs: tuple[str,...]=()
...
class CertifiedState:
    ...
    measurement_start: float; measurement_end: float; sample_size: int; certified_at: float; valid_until: float
    staleness_budget: float; environment_fingerprint: str; drift_detector: Optional[str]=None; drift_status: str="stable"
...
class ApprovalDecision:
    principal_id: str; role: str; mission_id: str; revision_id: str; intent_id: str; decision: str
    challenge_nonce: str; policy_revision: str; timestamp: float=field(default_factory=time.time)

@dataclass(frozen=True)
class AtomicEffect:
    effect_id: str; intent_id: str; depends_on: tuple[str,...]=(); state: IntentState=IntentState.PREPARED
    reversibility: str="unknown"; result_ref: Optional[str]=None; compensation_intent_id: Optional[str]=None
