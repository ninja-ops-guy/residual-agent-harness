...
def _run(argv, *, cwd=None):
    proc = subprocess.run(argv, cwd=cwd, env=_safe_env(), timeout=10,
...
class Px0Inspector:
    def __init__(self, binary="px0", *, required_version: str = PX0_VERSION, expected_binary_sha256: str | None = None):
        ...
        # Authenticate bytes before executing even a version probe. A rejected
        # executable must never run outside the inspection sandbox.
    ...
    def launch(self, workspace, *, port: int = 0, lsp: bool = False, open_browser: bool = True, sandbox: bool = True):
        if _sha256(self.binary) != self.binary_sha256:
            raise ContractError("px0 binary changed after qualification")
...
def main(argv=None):
    ...
    except (ContractError, OSError, ValueError, subprocess.TimeoutExpired) as exc:
