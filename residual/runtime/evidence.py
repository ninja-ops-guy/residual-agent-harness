...
def _scenario_descendant_cleanup() -> dict[str, Any]:
    if os.name != "posix":
        return {
            "scenario": "worker_process_group_descendant_cleanup",
            "observations": [{"status": "unsupported_platform"}],
            "expected": "posix_process_group_cleanup_or_explicit_qualification",
            "pass": True,
            "qualified": True,
        }
    ...
    async def run():
        ...
        controller = CancellationController(CancellationBudget(2.0))
        controller.track_process(process, name="worker-group",
    ...
    return {
        ...
        "observations": [{
            ...
            "descendants_clean": report.descendants_clean,
            "process_outcomes": list(report.process_outcomes),
            "survived": list(report.survived),
        }],
        "expected": "parent_and_descendants_terminated_within_budget",
        "pass": report.cancelled and report.descendants_clean and not report.survived,
        "qualified": False,
    }
...
if __name__ == "__main__":
    ...
    destination = repo / "evidence" / "runtime" / "runtime005_evidence.json"
    ...
    print(json.dumps({
        ...
        "all_pass": artifact["scenarios"]["all_pass"],
    }, sort_keys=True))
