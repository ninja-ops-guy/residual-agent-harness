...
def aggregate_metrics():
    ...
    return {
        ...
        "context_bytes_non_success_max": max(r["first_request_bytes"] for r in nonsuccess),
    }
...
def main() -> int:
    ...
    ap.add_argument("--output",default="runs/m6-spec-007k/evidence.json")
    a=ap.parse_args()
    ...
    with tempfile.TemporaryDirectory() as root:
        root=Path(root)
        src=root/"repo"
        ws.init_repo(src,{"README.md":"# M6-SPEC-007K branch-aware review fixture\n"})
        ...
        manifest={"name":"M6-SPEC-007K Branch-Aware Review","goal":"Originate and admit one evidence-grounded improvement proposal.","tasks":[{
        ...
        review={"approved":False,"findings":["mechanical admission failed"]} if errors else model_call(
            ...
            (GAP_REVIEW_SYSTEM if proposal.get("type") == "measurement_gap" else IMPROVEMENT_REVIEW_SYSTEM),
            REVIEW_SCHEMA,"local",TASK_ID,
            extensions=station.extensions(pid),
        )

        admitted=(not errors and review.get("approved") is True)
        verifier_revision=digest({"experiment":"M6-SPEC-007K","rules":"gap-closure-admission-v1"})
        receipt=None
        if admitted:
        ...
        print("M6_SPEC_007K_EVIDENCE="+json.dumps(evidence,sort_keys=True,separators=(",",":")))
