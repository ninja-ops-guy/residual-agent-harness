...
PROPOSAL_SCHEMA = {
    "oneOf": [
        ...
        },
        {
            ...
            "required": [
                "type", "question", "missing_metric", "why_needed", "proposed_measurement",
...
Choose the single most defensible measured improvement opportunity, or a MeasurementGap if evidence is insufficient.
...
def evidence_aware_schema(snapshot: dict) -> dict:
    schema=json.loads(json.dumps(PROPOSAL_SCHEMA))
    measured=sorted(snapshot["metrics"])
    invariants=sorted(ALLOWED_INVARIANTS)
    improvement=schema["oneOf"][0]["properties"]
    improvement["observation"]["properties"]["metric"]={"type":"string","enum":measured}
    improvement["observation"]["properties"]["comparison_metric"]={"type":"string","enum":measured}
    improvement["target_metrics"]["items"]={"type":"string","enum":measured}
    improvement["preserve_metrics"]["items"]={"type":"string","enum":measured}
    improvement["protected_invariants"]["items"]={"type":"string","enum":invariants}
    improvement["acceptance"]["items"]["properties"]["metric"]={"type":"string","enum":measured}
    gap=schema["oneOf"][1]["properties"]
    gap["missing_metric"]={"type":"string","not":{"enum":measured}}
    gap["preserve_invariants"]["items"]={"type":"string","enum":invariants}
    return schema
...
def verify(proposal: dict, snapshot: dict) -> list[str]:
    ...
    if kind=="improvement_spec":
        ...
        if not isinstance(missing,str) or not missing.strip() or missing in metrics: errors.append("missing_metric is not actually missing")
...
def main() -> int:
    ...
    ap.add_argument("--output",default="runs/m6-spec-007f/evidence.json")
    a=ap.parse_args()
    ...
    with tempfile.TemporaryDirectory() as root:
        ...
        proposal=model_call(
            ...
            SCIENTIST_SYSTEM,evidence_aware_schema(snapshot),"local","m6-discover-007d",
        ...
        verifier_revision=digest({"experiment":"M6-SPEC-007F","rules":"typed-admission-v1"})
        receipt=None
        if admitted:
        ...
        print("M6_SPEC_007F_EVIDENCE="+json.dumps(evidence,sort_keys=True,separators=(",",":")))
