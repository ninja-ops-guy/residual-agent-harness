...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as root:
        ...
        save_settings(station.store,{"local":{"kind":"ollama","model":a.model,"base_url":a.base_url,"output_token_field":"max_tokens"},"review_placement":"local","workers":1,"max_output_tokens":4096,"batch_max_passes":5,"batch_token_budget":30000,"batch_wall_clock_s":600})
        ...
        evidence={"schema_version":1,"experiment":"M6-SPEC-005","comparison_baseline_sha":"699e2869e294fe157b4bfd73a272057683a2f7e0","harness_sha":"e7488199ee238f04ef1547338199096389047b2e","intervention":"post-remediation integrated validation: prior-candidate repair context, transport/source clarity, shared five-attempt ceiling, 7B coding model, 4096 output tokens; acceptance and authority unchanged","baseline_sha":"699e2869e294fe157b4bfd73a272057683a2f7e0","provider":{"kind":"ollama","model":a.model,"base_url":a.base_url},"spec_sha256":hashlib.sha256(SPEC.encode()).hexdigest(),"batch":result,"metrics":metrics,"task":{"state":task["state"],"attempt":task["attempt"],"findings":task["findings"],"checks_result":task["checks_result"],"review":task.get("review"),"head_commit":task.get("head_commit"),"verification_receipt":task.get("verification_receipt")},"release_files":release_files,"generated_source":source,"generated_source_sha256":hashlib.sha256(source.encode()).hexdigest() if source else None,"export_error":export_error,"attempt_artifacts":attempt_artifacts,"events":[{"seq":e["seq"],"type":e["event_type"],"task_id":e["task_id"],"actor":e["actor"],"data":e["data"]} for e in events if e["event_type"] in {"task.transition","task.finding","usage.recorded","checks.completed","review.completed","integration.completed","release.exported"}]}
