...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as root:
        ...
        evidence={"schema_version":1,"experiment":"M6-SPEC-004","comparison_baseline_sha":"699e2869e294fe157b4bfd73a272057683a2f7e0","intervention":"repair attempts receive prior_candidate_files and runner transport is explicitly separated from file-language content; no acceptance-policy change","baseline_sha":"699e2869e294fe157b4bfd73a272057683a2f7e0","provider":{"kind":"ollama","model":a.model,"base_url":a.base_url},"spec_sha256":hashlib.sha256(SPEC.encode()).hexdigest(),"batch":result,"metrics":metrics,"task":{"state":task["state"],"attempt":task["attempt"],"findings":task["findings"],"checks_result":task["checks_result"],"review":task.get("review"),"head_commit":task.get("head_commit"),"verification_receipt":task.get("verification_receipt")},"release_files":release_files,"generated_source":source,"generated_source_sha256":hashlib.sha256(source.encode()).hexdigest() if source else None,"export_error":export_error,"attempt_artifacts":attempt_artifacts,"events":[{"seq":e["seq"],"type":e["event_type"],"task_id":e["task_id"],"actor":e["actor"],"data":e["data"]} for e in events if e["event_type"] in {"task.transition","task.finding","usage.recorded","checks.completed","review.completed","integration.completed","release.exported"}]}
