...
from residual.factory.reliability_m4 import aggregate_m4_faults
...
def main() -> int:
    ...
    (output / 'm4-fault-report.json').write_text(json.dumps(report, indent=2, sort_keys=True) + '\n')
    with (output / 'm4-fault-trials.jsonl').open('w') as stream:
