...
def verify(proposal: dict, snapshot: dict) -> list[str]:
    ...
    if kind=="improvement_spec":
        ...
        if not isinstance(missing,str) or not missing.strip() or missing in metrics: errors.append(f"missing_metric {missing!r} is already measured at {metrics.get(missing)!r}; choose improvement_spec or a genuinely absent metric")
...
def main() -> int:
    ...
    ap.add_argument("--output",default="runs/m6-spec-007e/evidence.json")
    a=ap.parse_args()
    ...
    with tempfile.TemporaryDirectory() as root:
        ...
        for attempt in range(1,4):
            ...
            if previous is not None:
                packet["previous_proposal"]=previous
                packet["mechanical_errors"]=errors
                packet["repair_instruction"]="Correct the prior typed proposal using only these mechanical findings. Do not invent measurements."
            proposal=model_call(
        ...
        verifier_revision=digest({"experiment":"M6-SPEC-007E","rules":"typed-admission-v1"})
        receipt=None
        if admitted:
