...
def main() -> int:
    ...
    ap.add_argument("--output",default="runs/m6-spec-007r/evidence.json")
    ...
    with tempfile.TemporaryDirectory() as root:
        root=Path(root)
        src=root/"repo"
        ws.init_repo(src,{"README.md":"# M6-SPEC-007R branch-aware review fixture\n"})
        ...
        manifest={"name":"M6-SPEC-007R Branch-Aware Review","goal":"Originate and admit one evidence-grounded improvement proposal.","tasks":[{
        ...
        binding=admission_binding(
            ...
            registry=registry,
            verifier_rules={"experiment":"M6-SPEC-007R","rules":"registry-aware-discovery-v1"},
        )
        receipt=None
        if admitted:
        ...
        print("M6_SPEC_007R_EVIDENCE="+json.dumps(evidence,sort_keys=True,separators=(",",":")))
