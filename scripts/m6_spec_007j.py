...
def main() -> int:
    ...
    ap.add_argument("--output",default="runs/m6-spec-007j/evidence.json")
    a=ap.parse_args()
    ...
    with tempfile.TemporaryDirectory() as root:
        root=Path(root)
        src=root/"repo"
        ws.init_repo(src,{"README.md":"# M6-SPEC-007J branch-aware review fixture\n"})
        ...
        manifest={"name":"M6-SPEC-007J Branch-Aware Review","goal":"Originate and admit one evidence-grounded improvement proposal.","tasks":[{
        ...
        verifier_revision=digest({"experiment":"M6-SPEC-007J","rules":"branch-aware-admission-v1"})
        receipt=None
        if admitted:
        ...
        print("M6_SPEC_007J_EVIDENCE="+json.dumps(evidence,sort_keys=True,separators=(",",":")))
