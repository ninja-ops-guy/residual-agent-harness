...
def _num(obj: dict[str, Any], key: str, name: str) -> float:
    value = obj.get(key)
...
def validate_evidence(evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    if observed_rpo > rpo_target:
        return _failed(f"observed RPO {observed_rpo:g}s exceeds target {rpo_target:g}s", commit, artifact_sha)
    if observed_rto > rto_target:
        return _failed(f"observed RTO {observed_rto:g}s exceeds target {rto_target:g}s", commit, artifact_sha)
