...
An observation and any ImprovementSpec target/preserve metrics must come from selected_evidence; use evidence_request for a metric you still need.
...
def main() -> int:
    ...
    ap.add_argument("--output",default="runs/m6-spec-007n/evidence.json")
    a=ap.parse_args()
    ...
    with tempfile.TemporaryDirectory() as root:
        root=Path(root)
        src=root/"repo"
        ws.init_repo(src,{"README.md":"# M6-SPEC-007N branch-aware review fixture\n"})
        ...
        manifest={"name":"M6-SPEC-007N Branch-Aware Review","goal":"Originate and admit one evidence-grounded improvement proposal.","tasks":[{
        ...
        scientist_outputs=[]
        resolution_history=[{"status":"actively_selected","metrics":selected_evidence}]
        proposal=None
        ...
        verifier_revision=digest({"experiment":"M6-SPEC-007N","rules":"active-evidence-query-admission-v1"})
        receipt=None
        if admitted:
        ...
        print("M6_SPEC_007N_EVIDENCE="+json.dumps(evidence,sort_keys=True,separators=(",",":")))
