...
d=s.to_dict()
if d['target_metrics']!=['task_success_rate','repair_attempts']: raise AssertionError('target_metrics must serialize as ordered list')
if d['preserve_metrics']!=['verification_integrity']: raise AssertionError('preserve_metrics must serialize as ordered list')
if d['protected_invariants']!=['M4','evidence_integrity','promotion_authority']: raise AssertionError('protected_invariants must serialize as ordered list')
if d['human_approval_required'] is not True: raise AssertionError('human approval must default true')
d['acceptance']['success_rate_min']=0
if s.to_dict()['acceptance']['success_rate_min']!=0.9: raise AssertionError('to_dict must defensively copy acceptance')
c=s.canonical_json()
expected=json.dumps(s.to_dict(),sort_keys=True,separators=(',',':'),ensure_ascii=False)
if not isinstance(c,str): raise AssertionError('canonical_json must return str, got '+type(c).__name__)
if c!=expected: raise AssertionError('canonical_json mismatch expected='+repr(expected)+' got='+repr(c))
expected_hash=hashlib.sha256(c.encode('utf-8')).hexdigest()
if s.sha256()!=expected_hash: raise AssertionError('sha256 mismatch expected='+expected_hash+' got='+repr(s.sha256()))
if s.sha256()!=ImprovementSpec(**kw).sha256(): raise AssertionError('same contract must have stable sha256 identity')
try:
...
bad=[
 ('blank improvement_id',dict(kw, improvement_id=' ')),
 ('blank observation',dict(kw, observation='\t')),
 ('blank hypothesis',dict(kw, hypothesis='')),
 ('empty target_metrics',dict(kw, target_metrics=())),
 ('empty preserve_metrics',dict(kw, preserve_metrics=())),
 ('empty protected_invariants',dict(kw, protected_invariants=())),
 ('blank target metric',dict(kw, target_metrics=('ok',' '))),
 ('blank preserve metric',dict(kw, preserve_metrics=(' ',))),
 ('blank protected invariant',dict(kw, protected_invariants=('M4',''))),
 ('empty acceptance',dict(kw, acceptance={})),
 ('human approval disabled',dict(kw, human_approval_required=False)),
]
for label,case in bad:
 try:
  ...
  raise AssertionError('invalid ImprovementSpec accepted: '+label)
"""
...
def main() -> int:
    ...
    p.add_argument("--output", default="runs/m6-roadmap-ship-002/improvementspec-evidence.json")
    a=p.parse_args()
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        manifest={
            ...
            "tasks":[{
                ...
                "context":["residual/core.py","residual/goalspec.py"],
                "depends_on":[],
                "route":"local",
                "checks":[
