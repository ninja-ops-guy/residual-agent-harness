...
def main() -> int:
    ...
    ap.add_argument("--output",default="runs/m6-spec-007i/evidence.json")
    a=ap.parse_args()
    ...
    with tempfile.TemporaryDirectory() as root:
        root=Path(root)
        src=root/"repo"
        ws.init_repo(src,{"README.md":"# M6-SPEC-007I branch-aware review fixture\n"})
        ...
        manifest={"name":"M6-SPEC-007I Branch-Aware Review","goal":"Originate and admit one evidence-grounded improvement proposal.","tasks":[{
            "id":"m6-discover-007i","title":"Typed discovery","instruction":"typed structured proposal",
        ...
        review={"approved":False,"findings":["mechanical admission failed"]} if errors else model_call(
            ...
            REVIEW_SCHEMA,"local","m6-discover-007i",
            extensions=station.extensions(pid),
        )
        ...
        print("M6_SPEC_007I_EVIDENCE="+json.dumps(evidence,sort_keys=True,separators=(",",":")))
