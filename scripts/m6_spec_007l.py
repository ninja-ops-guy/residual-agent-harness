...
def main() -> int:
    ...
    ap.add_argument("--output",default="runs/m6-spec-007l/evidence.json")
    a=ap.parse_args()
    ...
    with tempfile.TemporaryDirectory() as root:
        root=Path(root)
        src=root/"repo"
        ws.init_repo(src,{"README.md":"# M6-SPEC-007L branch-aware review fixture\n"})
        ...
        manifest={"name":"M6-SPEC-007L Branch-Aware Review","goal":"Originate and admit one evidence-grounded improvement proposal.","tasks":[{
        ...
        verifier_revision=digest({"experiment":"M6-SPEC-007L","rules":"gap-closure-admission-v1"})
        receipt=None
        if admitted:
        ...
        print("M6_SPEC_007L_EVIDENCE="+json.dumps(evidence,sort_keys=True,separators=(",",":")))
