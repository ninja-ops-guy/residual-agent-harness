...
and SCHEMA-ERRATUM-001):
  ...
  - authority defaults to requested=[], granted=[], violation=False ONLY where the
    ...
    (preflight SS2.1), flagged in provenance.authority_default_assumed.
...
Contamination groups:
  ...
  VQ   : vq:<case_id prefix>:<part-file>  (frozen workload lineage per part)
  OBS  : obs006-fixture-v1                (single fixture = single group)
  DSM  : dsm004:<journal-path lineage>    (schedule/recovery lineage; retries,
         duplicates, replays of a lineage share its group)
...
(GitHub MCP only); logic is pure parsing + field mapping.
"""
...
def base_record(obs_id, source_file, source_class, contamination_group,
    ...
    prov = {
        ...
        "incident_id": None,
        "generation": None,
        "artifact_digests": ["sha256:" + SOURCE_SHAS[source_file]] if source_file in SOURCE_SHAS else [],
...
def convert_dsm004(root):
    ...
    for journal_path, lines in sorted(doc["raw_event_logs"].items()):
        group = f"dsm004:{journal_path.rsplit('/', 1)[0]}"  # schedule/recovery lineage
        for line in lines:
            ev = json.loads(line)
...
def convert_vq(root):
    out = []
    for src in [p for p in SOURCE_SHAS if p.startswith("evidence/vq/outcomes-")]:
...
def main():
    ...
    records, rejects = [], {}
    for fn, src_key in [
        (convert_otx, "evidence/otx/observations.jsonl"),
        (convert_obs006, "evidence/obs/obs006-evidence.json"),
        (convert_dsm004, "evidence/dsm/dsm-004-evidence.json"),
        (convert_runtime005, "evidence/runtime/runtime005_evidence.json"),
        (convert_vq, "evidence/vq"),
    ]:
