...
Checks (fail-closed: any ERROR finding exits non-zero):
  schema        required fields / enums of observation.schema.json
                (slm-observation-v0) are satisfied
...
def check_schema(rec):
    ...
    if rec.get("schema_version") != SCHEMA_VERSION:
        errs.append("schema_version != %r" % SCHEMA_VERSION)
...
def check_provenance(rec):
    ...
    if not rec.get("timestamp"):
        errs.append("missing timestamp (unreplayable)")
    return errs
