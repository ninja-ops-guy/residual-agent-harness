import {PROTOCOL, validId, validInference, validModel, bounded, errorCode, textReply} from './provider-session.js';
...
sign.addEventListener('click', () => {
  ...
  Promise.race([auth, new Promise((_, reject) => { timer = setTimeout(() => reject(new Error('timeout')), 60000); })])
    .then(() => { if (!sdk.auth.isSignedIn()) throw new Error('not_signed_in'); tell('Connected. Return to Mission Control; keep this tab open. Model access is not yet tested.'); })
...
async function receive(m) {
  ...
  try {
    const result = await Promise.race([
      sdk.ai.chat(m.messages, {model: m.model, max_tokens: m.max_output_tokens, stream: false}),
    ...
    const text = textReply(result), u = result?.usage || {};
    ...
    tell('Model response returned to the guest. RESIDUAL—not this provider tab—checks its contract.');
  } catch (error) {
    reply({ok: false, error: error?.message === 'provider_timeout' ? 'provider_timeout' : 'provider_error'});
    tell('Model request failed or timed out. Check model availability/allowance in Puter. A timed-out request may still be billed.');
