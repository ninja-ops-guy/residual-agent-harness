...
export function mountMissionControl(host){
  ...
  const wrapped={...host,run:async request=>{let next={...request};if(request.mode==='build'){const previous=document.querySelector('#mc-inline-preview-frame');if(previous)previous.id=`mc-inline-preview-frame-r${Math.max(1,state.revision)}`;next.conversation_id=state.id;if(state.continueFrom)next.parent_mission_id=state.continueFrom;}pending.add(request.id);append(state,{role:'user',text:request.prompt,mission_id:request.id,mode:request.mode});queueMicrotask(()=>{const prompt=document.querySelector('#mc-prompt'),controls=document.querySelector('#mc-composer details');if(prompt&&prompt.value===request.prompt)prompt.value='';if(controls)controls.open=false});return host.run(next)}};
