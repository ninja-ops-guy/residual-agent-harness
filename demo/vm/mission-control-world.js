...
export function mountMissionControl(host){
  let id=localStorage.getItem(CURRENT);if(!validCid(id))id=cid();
  let state=load(id), pending=new Set();
  const wrapped={...host,run:async request=>{let next={...request};if(request.mode==='build'){next.conversation_id=state.id;if(state.continueFrom)next.parent_mission_id=state.continueFrom;pending.add(request.id);append(state,{role:'user',text:request.prompt,mission_id:request.id,mode:'build'});}return host.run(next)}};
  ...
  function restore(){chat.replaceChildren();if(!state.turns.length)bubble(chat,'assistant','Give the harness a job. Follow up naturally; accepted build artifacts stay attached to this conversation until you detach them or start a new chat.');for(const turn of state.turns){const node=bubble(chat,turn.role,turn.text);if(turn.role==='assistant'&&turn.mission_id===state.lastBuildMission&&state.lastBundle)renderPreview(node,state.lastBundle,{frameId:'mc-restored-preview-frame'});}chat.scrollTop=chat.scrollHeight;status();fillHistory()}
  ...
  restore();
  function capture(event){if(!event||!pending.has(event.mission_id))return;if(event.kind==='mission_error'){append(state,{role:'assistant',text:'Mission failed without a verified result.',mission_id:event.mission_id,status:'INCOMPLETE'});pending.delete(event.mission_id);status();return}if(event.kind!=='mission_finished')return;const data=event.data||{},build=data.execution==='generated_artifacts',bundle=data.result?.values?.build;let text=build?(bundle?.summary||'Build completed.'):data.result?.values?.answer?.text;if(!text)text=data.status?`Mission ${String(data.status).toLowerCase()}.`:'Mission completed.';append(state,{role:'assistant',text,mission_id:event.mission_id,status:data.status,mode:build?'build':'other'});if(build&&data.result?.success&&bundle){state.lastBuildMission=event.mission_id;state.continueFrom=event.mission_id;state.lastBundle=bundle;state.revision=data.lineage?.revision||state.revision+1;save(state)}pending.delete(event.mission_id);status();fillHistory()}
