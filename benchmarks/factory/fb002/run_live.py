...
class FB002OllamaEngine:
    ...
    def execute(self, task: TaskSpec, context: ContextAssembly) -> EngineResult:
        ...
        if prompt is None:
            raise ValueError('unknown FB002 task')
        dependency_context = context.values.get('dependencies', {})
        dep_note = ''
        if dependency_context:
            dep_note = '\nVerified dependency implementations are available; honor their specified interfaces exactly.'
        request = ChatRequest(
            model=self.model,
            messages=(
                Message(Role.SYSTEM, 'You are executing a deterministic software-engineering benchmark. Return only valid JSON matching the schema. Do not use markdown fences.'),
                Message(Role.USER, prompt + dep_note),
            ),
            temperature=0,
            max_tokens=700,
            seed=self.seed,
        ...
        if not isinstance(payload, dict) or set(payload) != {'code'} or not isinstance(payload['code'], str):
            raise ValueError('FB002 model output violated schema')
        ...
        return EngineResult(
            ...
            raw_metadata={
                ...
                'gpu_time_ms': float(raw.get('eval_duration') or 0) / 1_000_000,
            },
        )


def main() -> int:
    p = argparse.ArgumentParser(description='Execute one measured FB002 configuration/run against Ollama')
    ...
    engine = FB002OllamaEngine(args.model, args.model_version, args.base_url, workload.seed)
    ...
    with tempfile.TemporaryDirectory(prefix=f'residual-fb002-{args.config}-{args.run}-') as td:
        repo = Path(td) / 'repo'
        ...
        def verifier(task_id: str, _acceptance: tuple[str, ...], candidate) -> bool:
            return isinstance(candidate, str) and verify_candidate(task_id, candidate)

        def finalizer(_workload, outputs, _config, _run_index) -> FinalizedRun:
            commit, passed, total = finalize(repo, dict(outputs))
            if commit != EXPECTED_OUTPUT_COMMIT or commit != _workload.expected_output_commit:
                raise ValueError('FB002 final output commit drift')
            return FinalizedRun(commit, passed, total, 0)

        driver = EngineBackedEvaluationDriver(engine, finalizer=finalizer, verifier=verifier, fixed_workers=4, max_workers=6)
        measurement = driver.run(workload, args.config, args.run, lambda _event: None)

    out = Path(args.output); out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(measurement.to_dict(), indent=2, sort_keys=True) + '\n', encoding='utf-8')
    return 0
