...
ALLOWED_IMPORTS = {
    "text": {"re"}, "retry": set(), "paths": {"pathlib"}, "config": set(),
    "summary": {"service.text", "service.config", ".text", ".config"},
    "banner": {"service.retry", "service.paths", ".retry", ".paths"},
}
...
def create_fixture(repo: Path, *, known_good: bool = False) -> tuple[str, str | None]:
    ...
    if input_commit != INPUT_COMMIT:
        raise RuntimeError(f"FB002 input commit drift: {input_commit}")
    if not known_good:
    ...
    if output_commit != EXPECTED_OUTPUT_COMMIT:
        raise RuntimeError(f"FB002 output commit drift: {output_commit}")
...
def _validate_ast(task_id: str, source: str) -> None:
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.Global, ast.Nonlocal, ast.AsyncFunctionDef, ast.ClassDef, ast.Lambda)):
            raise ValueError("candidate uses disallowed construct")
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id in banned_calls:
            raise ValueError("candidate uses disallowed call")
        if isinstance(node, ast.Attribute) and node.attr.startswith("_"):
            raise ValueError("candidate uses private attribute")
...
def verify_candidate(task_id: str, source: str) -> bool:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="fb002-verify-") as td:
            ...
            target, _canonical = REFERENCE[task_id]; (repo / target).write_text(source.rstrip() + "\n", encoding="utf-8")
            code = "import sys; sys.path.insert(0, r'%s'); %s" % (str(repo), TEST_SNIPPETS[task_id])
            p = subprocess.run([sys.executable, "-c", code], cwd=repo, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=5)
            return p.returncode == 0
    except (ValueError, SyntaxError, subprocess.TimeoutExpired, RuntimeError):
        return False


def finalize(repo: Path, accepted: dict[str, str]) -> tuple[str, int, int]:
    if set(accepted) != set(REFERENCE):
        raise ValueError("FB002 requires all six accepted tasks")
    for task_id, source in accepted.items():
        if not verify_candidate(task_id, source):
            raise ValueError(f"unverified candidate: {task_id}")
    for task_id, (name, canonical_source) in REFERENCE.items():
        (repo / name).write_text(canonical_source, encoding="utf-8")
    (repo / "service/__init__.py").write_text(INIT_REFERENCE, encoding="utf-8")
    commit = _commit(repo, "FB002 known-good output")
    checks = []
