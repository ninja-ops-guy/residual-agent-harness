...
from common import DEPENDENCIES, EXPECTED_OUTPUT_COMMIT, INPUT_COMMIT, PROMPTS, create_fixture  # noqa: E402
...
def main() -> int:
    p = argparse.ArgumentParser(description='Prepare Frozen Benchmark 002 for a local Ollama model')
    ...
    if fixture.exists():
        raise SystemExit(f'remove stale {fixture} before preparing FB002')
    try:
    ...
    tasks = []
    for task_id in ('text', 'retry', 'paths', 'config', 'summary', 'banner'):
        tasks.append({
            'task_id': task_id,
            'requirement_ids': [f'FB002-{task_id.upper()}'],
            'depends_on': list(DEPENDENCIES[task_id]),
            'acceptance': [f'semantic-tests:{task_id}'],
        })
    workload = {
        'schema_version': 'factory-evaluation-v1',
        'workload_id': 'fb002-multifile-code',
        'requirements': [f'FB002-{x.upper()}' for x in ('text','retry','paths','config','summary','banner')],
        'tasks': tasks,
    ...
    print(json.dumps({'workload': str(Path(args.output).resolve()), 'station_key': str(key.resolve()), 'model_version': version}, indent=2))
    return 0
