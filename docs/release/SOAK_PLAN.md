...
- **Duration:** 30 consecutive days (N9-R9), `--days 30`. The driver is
  resumable: `soak-state.json` is saved after every day and its digest,
  configured seed/load/duration, journal head, complete journal SHA-256 and
  record count are bound into an HMAC-signed `soak-checkpoint.json` before a
  later invocation is allowed to resume.
...
- **Sampling cadence:** one typed `DAY` record per soak day. The journal chain,
  signed checkpoint and prior retention manifest are verified before resumed
  work is accepted.
...
2. **Crash:** the harness itself raises; the run halts and evidence already
   committed remains available for investigation.
...
- `retention-manifest.json` — SHA-256 of every retained file plus the journal
  head and record count. Schema v2 explicitly records `COMPLETE`, `PAUSED`, or
  `STOPPED`.

**Retention rule:** retain the entire directory unmodified. Before citing any
artifact, verify the checkpoint HMAC, the journal chain/head/count and every
file digest against the retention manifest. Internal hash links alone do not
prove that a tail was not truncated; the signed checkpoint is the external
resume boundary used by this procedure.
...
pause/resume, stop criteria, checkpoint tamper detection and retention. Those
runs are **not** 30-day soak evidence and cannot be cited as production or
release reliability. The actual 30-day wall-clock run and any substitution of
the simulator executor (`SoakHarness._execute_task`) with a live Station
pipeline remain release-candidate work and must be retained at the exact
qualified revision.
