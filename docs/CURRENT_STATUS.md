...
Open **#356** was first evaluated at predecessor head **`95ab297cb7a4b82a54fe81db11b0340d3ae030fe`**. That product-path candidate changes only `residual/sandbox/bwrap.py`, `residual/supplychain/sandbox.py`, `tests/test_sandbox.py`, and `tests/test_supplychain_sandbox_runtime.py`; it does not change ownership baselines, protected Factory/M4 bytes, verifier authority, evidence schemas, or skip policy. The candidate keeps the existing minimal bwrap runtime roots, chooses a jail-visible `/usr/bin/python3` for supply-chain execution instead of widening host mounts, and remounts only the synthetic sandbox root read-only after explicit mounts are constructed.

At exact predecessor head `95ab297c...`, #356 `RESIDUAL Qualification v1` run **`35484064567`, attempt 1, is PASS**. Its final manifest is bound to source commit `95ab297c...`, tree `c7c7d6f4d626af76a80809f587fa6f0c82e9728c`, contains all 25 required gates as **PASS**, has no missing gates, and records no gate skips or unknowns. The deterministic gate itself is **PASS** with **1865 passed / 387 subtests passed / 0 skipped**, `unknown_count: 0`; retained deterministic artifact `qualification-v1-deterministic-7b651be81050bff064b27c3ed90ec168eaf0c9fc`, artifact id `10596342910`, SHA-256 `ac1b5e7fef9fe9aa10e2bc6603f9306847ca215d40d6458dd83cc641529718af`. The final manifest artifact is `qualification-v1-final-7b651be81050bff064b27c3ed90ec168eaf0c9fc`, artifact id `10596218366`, SHA-256 `8889ef7b9c87c2cd7553dfedea951ecf42a42e01ec29c7a67b45ef84bc895fea`.

That predecessor qualification PASS remains bounded to `95ab297c...`. On the same predecessor head, Control Plane, Factory ownership, measured-evaluation binding, clean install, Command Station, and PR-head GitHub Pages run **`35484064541`** were **PASS**, while **Controller/provider contracts run `35484064575` was FAIL** because its Python 3.13 matrix job `106007034610` failed at `Compile and test without model credentials`; Python 3.11 and 3.12 passed. Successor #357 retained the lower-level result as **1 failed / 1175 run / 22 skipped**, with the sole failure `RuntimeJournalWriterAdmissionTests.test_constructor_persistent_busy_is_bounded_and_fail_closed` observing one constructor `BEGIN` attempt where the test required more than one. That predecessor failure remains historical exact-head evidence.

Merged **#357** was the focused one-file test-instrumentation successor. At its own exact head **`a18fdf2041f314b7285975e1bf621760f6a6fe35`**, it changed only `tests/test_release_stabilization_runtime_journal.py`: the 120 ms persistent-constructor-contention deadline and fail-closed bounds remained unchanged, while the test stopped requiring a second `BEGIN` when the first initialization cycle can consume the whole budget on Python 3.13; the separate transient-contention test continues to pin retry behavior. #357 changed no RuntimeJournal implementation, timeout, verifier authority, Factory/M4 protected byte, ownership baseline, evidence schema, sandbox control, or qualification skip semantics.

On exact #357 head, `RESIDUAL Qualification v1` run **`35487109155`, attempt 1, was PASS** and retained artifacts include deterministic artifact id **`10598546143`**, SHA-256 **`badcfafe6ef5346af4d7be4fe501802f0e25994b1290c99a7577e8d7b45689ba`**, and final-manifest artifact id **`10598465702`**, SHA-256 **`d95e57cdea360fa777c9058c98d025fa3ffc00a6208df9c1ffc129d2f0dbcf0a`**. Controller/provider run **`35487109176` was PASS** across Python 3.11, 3.12, and 3.13, including the Python 3.13 `Compile and test without model credentials` step. Control Plane, Factory ownership, measured-evaluation binding, clean install, and Command Station workflows were also **PASS**; Vercel was **PASS** and the ordinary Aikido code scan was **PASS**, while Aikido deep review was **SKIPPED / unavailable** for exhausted credits. PR-Agent advisory run **`35487109150`, attempt 2, was PASS**, including the repository's `Verify substantive advisory review was published` step; attempt 1 remains historical **FAIL** evidence and is not erased.

#357 has now been **merged into #356's branch, not into `main`**, producing current #356 head **`de74ae8f1b287e1b56a9c0c270404c420eebc431`**. The merge commit contains the one-file #357 test-instrumentation change; it does not by itself accept the sandbox repair into production or transfer #357's prior qualification to the changed #356 head.

Fresh exact-head evidence for #356 at `de74ae8f...` is mixed. `RESIDUAL Qualification v1` run **`35514396063`, attempt 1, is FAIL**. The deterministic job is **PASS**, including `Provision real bubblewrap sandbox` and `Full deterministic regression gate`; the M4 job is **PASS**; Chromium and WebKit cross-browser jobs are **PASS**. The **Firefox** job `106087768823` is **FAIL** at `Cross-browser Station user journey`, and the aggregate job then fails closed while building the release qualification manifest. The retained workflow summary does not establish a reviewed lower-level Firefox invariant or root cause, so the cause remains **UNKNOWN** rather than being inferred.

On that same current #356 head, Control Plane, Factory ownership, measured-evaluation binding, clean install, PR-Agent advisory, Command Station, Controller/provider, PR-head GitHub Pages, and Vercel are **PASS**. Protected maintainer approval is **FAIL**. #356 is therefore **UNACCEPTED / FAIL on exact-head qualification** despite its current-head product/advisory greens and predecessor qualification successes. #357's historical PASS remains revision-bound to `a18fdf20...` and does not overwrite the current #356 FAIL.

The older `aeba9962...` run `35448856959` remains historical exact-revision **FAIL** evidence. Earlier #152, #355, predecessor #356, and #357 results remain bound to their exact heads and are not inherited forward. Any repair of the current #356 Firefox failure requires a changed exact head and fresh qualification; only after #356 itself qualifies may the repair move into #355, which must then be freshly qualified before any integration into #152.
...
- **#356:** open product-path sandbox repair candidate at current exact head **`de74ae8f1b287e1b56a9c0c270404c420eebc431`** after merged successor #357. Exact-head Qualification-v1 run `35514396063` is **FAIL** because Firefox job `106087768823` fails at `Cross-browser Station user journey`; deterministic, M4, Chromium and WebKit qualification jobs are **PASS**, and the aggregate then fails closed. The lower-level Firefox cause remains **UNKNOWN** from retained workflow summary. Control Plane, Factory ownership, measured-evaluation binding, clean install, Controller/provider, Command Station, PR-Agent advisory, PR-head Pages and Vercel are **PASS**, while protected maintainer approval is **FAIL**. #356 remains **UNACCEPTED / FAIL on exact-head qualification**.
- **#357:** closed/merged one-file test-instrumentation successor, merged into #356's branch rather than `main`. Its own exact head **`a18fdf2041f314b7285975e1bf621760f6a6fe35`** retains Qualification-v1 `35487109155`, Controller/provider `35487109176`, Control Plane, Factory ownership, measured-evaluation binding, clean install, Command Station, Vercel, and PR-Agent advisory run `35487109150` attempt 2 as **PASS**; attempt 1 remains historical **FAIL** evidence. Those results remain bound to #357's exact head and do not qualify current #356 `de74ae8f...`, whose fresh qualification is FAIL.
...
3. **Carry #152 qualification convergence through #355 and current #356 without rewriting retained evidence.** Exact `6cede252...` run `35481689839` remains **FAIL** with the Ubuntu-24 `RTM_NEWADDR` startup signature. Exact #355 head `a2a95116...` removes that startup signature but run `35482158683` remains **FAIL** on nine deterministic sandbox/security regressions. Predecessor #356 head `95ab297c...` has a scoped Qualification-v1 and PR-head Pages **PASS**, but required Controller/provider CI is historical **FAIL** on its Python 3.13 timing assertion. #357 exact head `a18fdf20...` repaired only that test instrumentation and retained scoped Qualification-v1, Controller/provider, and PR-Agent substantive-advisory **PASS**; #357 is now merged into #356's branch, producing current #356 head `de74ae8f...`. Fresh current-head Qualification-v1 run `35514396063` is **FAIL** at the Firefox cross-browser Station journey even though deterministic, M4, Chromium/WebKit qualification jobs and the checked ordinary product/advisory lanes pass; the lower-level Firefox cause remains **UNKNOWN**. Do not transfer #357 evidence forward: repair/investigate the current #356 Firefox failure, freshly qualify any changed #356 head, then integrate only a qualifying #356 into #355 and freshly qualify #355 before any integration into #152. Preserve every predecessor FAIL/PASS as revision-bound evidence.
...
6. **Repair and requalify current exact-head failures before treating affected candidates as merge-ready.** #340 and #349 still have their previously recorded product-test failures. Current #356 no longer carries the predecessor Python 3.13 product-CI failure after #357 integration, but its exact-head Qualification-v1 run `35514396063` is **FAIL** on the Firefox cross-browser Station journey; the lower-level cause is **UNKNOWN** until retained evidence identifies it. A changed #356 head requires fresh exact-head qualification.
...
15. **Keep refreshed #198 fail-closed and exact-head-bound.** Its source/update-path candidate is unaccepted because Factory runtime evidence run `35489733181` is **FAIL** on the persistent-constructor-busy timing assertion. #357's historical PASS does not qualify #198; any integrated repair requires a changed head and fresh exact-head CI/attestation before the update command can be treated as accepted behavior.
