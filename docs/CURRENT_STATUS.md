...
The current interpretation is a guest/interpreter lifecycle risk, not a proven application-level defect. PR #136 is the current-main mitigation candidate: it keeps Mission Control on one persistent in-guest Python worker, serializes missions, validates worker PID/FIFO identity, keeps provider mailbox semantics unchanged, and fails closed on unexpected worker corruption. Its own boundary is correct: this is a lifecycle-risk mitigation, **not** a proven root-cause fix.
