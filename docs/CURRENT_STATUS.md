...
3. **Repair and freshly qualify #152's protected qualification path without rewriting either retained FAIL.** Exact `6cede252...` run `35481689839` is **FAIL** with the Ubuntu-24 `RTM_NEWADDR` namespace-startup signature. Exact #355 head `a2a95116...` removes that observed startup signature but run `35482158683` is still **FAIL**, now on nine deterministic sandbox/security regressions. Review and repair that changed failure set without weakening containment or assertions; then require a fresh exact-head qualification on the resulting `testing/qualification-v1` head, preserve both predecessor FAILs, and require explicit protected review/attestation before merge.
