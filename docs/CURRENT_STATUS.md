...
That qualification PASS is bounded to #356's exact head and does not make the candidate merge-ready by itself. On the same head, Control Plane, Factory ownership, measured-evaluation binding, clean install, and Command Station workflows are **PASS**, while **Controller/provider contracts is FAIL** because its Python 3.13 matrix job fails at `Compile and test without model credentials`; Python 3.11 and 3.12 jobs pass. The lower-level Python 3.13 failure invariant is **UNKNOWN** from the retained workflow summary checked here. PR-Agent advisory is also **FAIL**, and the PR-head Pages workflow was still in progress at this status check. #356 therefore remains **UNACCEPTED / FAIL on required exact-head product CI** despite the scoped Qualification-v1 PASS. It must not be merged into #355 until the failing required product lane is understood/repaired and the resulting exact head is freshly requalified.

The older `aeba9962...` run `35448856959` remains historical exact-revision **FAIL** evidence. Earlier #152 and #355 results remain bound to their exact heads and are not inherited forward. The #356 PASS does not rewrite either predecessor failure and cannot be inherited by a later #355 or #152 head after merge/rebase.
...
- **#356:** open product-path sandbox repair candidate at exact head **`95ab297cb7a4b82a54fe81db11b0340d3ae030fe`**. Exact-head Qualification-v1 run `35484064567` is **PASS** with all 25 required gates present and PASS, including deterministic **1865 passed / 387 subtests / 0 skipped**. This is a scoped repair-head PASS only. Required Controller/provider CI is **FAIL** on the Python 3.13 matrix job while Python 3.11/3.12 pass; the lower-level cause remains **UNKNOWN** from the retained workflow summary. PR-Agent advisory is **FAIL**, and the candidate is **UNACCEPTED / not merge-ready** until required product CI is green and the resulting exact head is freshly qualified.
...
3. **Carry #152 qualification convergence through the #355 → #356 repair chain without rewriting retained evidence.** Exact `6cede252...` run `35481689839` remains **FAIL** with the Ubuntu-24 `RTM_NEWADDR` startup signature. Exact #355 head `a2a95116...` removes that startup signature but run `35482158683` remains **FAIL** on nine deterministic sandbox/security regressions. Exact #356 head `95ab297c...` has a scoped Qualification-v1 **PASS** with all 25 required gates PASS and deterministic 1865/387/0-skipped, but required Controller/provider CI is **FAIL** on Python 3.13. Repair or explain that exact-head product failure without weakening contracts; then require a fresh exact-head qualification on the resulting #356 head, merge into #355 only after its own required gates are satisfied, freshly qualify #355 after the merge, then repeat the same evidence/attestation discipline before integrating into #152. Preserve every predecessor FAIL/PASS as revision-bound evidence.
...
6. **Repair and requalify current exact-head product-test failures** before treating #340, #349, or #356 as merge-ready.
