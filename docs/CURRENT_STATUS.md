...
The strongest discriminator now removes Mission Control entirely. Exact diagnostic head `d0983b452ed1be23fd588c8b3156cf5ff6c37ae2` ran `35121528538` against the published accepted-main WebVM with `/tmp/residual-workbench.pid` required to remain absent. A **single guest CPython process** then executed 400 repetitions of `time.sleep(0.05)`:
...
For this symptom, the retained evidence therefore rules out Mission Control workbench logic, the persistent worker, coexistence with a second long-lived guest CPython interpreter, and background execution specifically as **necessary conditions**. A single foreground guest CPython process reproduces the failure under the published WebVM environment. The strongest supported classification is a **WebVM guest CPython/time-runtime defect under the deployed environment/path**. The underlying mechanism is still **UNKNOWN**: the evidence does not yet establish why sleep 273 is the boundary, whether the defect is in guest CPython, time/clock emulation, WebVM integration or another lower-level component, or whether the historical `_sha512`, impossible-constructor and `munmap_chunk()` failures share this cause. Their relationship remains **UNKNOWN**.

PR #133 has moved to lower-level time/runtime discrimination rather than patching Mission Control. The active diagnostic direction varies sleep duration, records monotonic/wall/performance-clock observations and compares against a native i386 CPython control. Those candidate diagnostics are not accepted-main qualification gates; first-attempt results must be retained without retry-to-green substitution.
...
| Mission Control / WebVM | Implemented product/demo surface; **long-run reliability gate open** | PR #136 is integrated and exact merged revision `3a41dc1e...` passed its first public desktop+narrow release attempt. Current-main diagnostics reproduce the same `_PyTime_t` overflow in a single foreground guest CPython sleep loop with Mission Control absent, so the symptom is below the Mission Control worker path. #120/#126 remain open; the lower-level mechanism and relationship to the historical corruption family remain `UNKNOWN`. |
...
- **PR #133 — WebVM runtime diagnostics.** The diagnostic lane targets published `main@3a41dc1e...` and remains unmerged. Worker and independent-canary experiments first reproduced `OverflowError: timestamp too large to convert to C _PyTime_t`; no-worker foreground and background single-CPython arms then both reproduced the same overflow at sleep 273 with Mission Control absent. This rules out Mission Control and a second guest interpreter as necessary for this symptom. The strongest current classification is a deployed WebVM guest CPython/time-runtime defect, while the exact mechanism and historical corruption-family relationship remain `UNKNOWN`. Lower-level duration/clock/native-control diagnostics are now the appropriate next evidence.
...
- the WebVM guest time/runtime fault or historical lifecycle corruption has been root-caused or shown to have an acceptable recurrence rate;
...
5. **Move the reproducible `_PyTime_t` investigation below Mission Control.** Preserve the worker failures, bounded shell-only non-reproduction, invalid first canary classification, repaired canary failures, and no-worker foreground/background failures. Vary sleep duration, record guest clocks and compare equivalent native controls to determine whether the deterministic sleep-273 boundary follows call count, cumulative requested sleep, elapsed guest time or another runtime state variable. Keep the historical `_sha512`/heap-corruption relationship `UNKNOWN` unless evidence links them.
