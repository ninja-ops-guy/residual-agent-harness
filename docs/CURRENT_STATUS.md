...
At this check, #355 exact-head Factory ownership, Control Plane, measured-evaluation binding and clean-install workflows are **PASS**; PR-Agent advisory is **FAIL**; Command Station, Controller/provider and Qualification-v1 are still **in progress**. Therefore #355 repair qualification is **UNKNOWN / pending**, and the retained `6cede252...` FAIL must not be rerun or rewritten merely to obtain green.

The older `aeba9962...` run `35448856959` remains historical exact-revision **FAIL** evidence. Earlier #152 results remain bound to their exact heads and are not inherited forward.
...
- **#355:** open qualification-runner repair candidate against `testing/qualification-v1`, exact head **`a2a95116b840483bae421b6f8e1338a151a42ec4`**. It pins only the deterministic Qualification-v1 job to `ubuntu-22.04` and adds a fail-closed real bubblewrap namespace-capability probe plus regression assertions. It does not change runtime sandbox or protected Factory/M4 implementation/evidence schemas. Exact-head Qualification-v1 remains **UNKNOWN / pending** while its run is in progress; the predecessor `6cede252...` result remains retained **FAIL** evidence.
...
3. **Repair and freshly qualify #152's current protected qualification path without rewriting the retained failure.** Exact head `6cede252...` has run `35481689839` **FAIL** at the deterministic regression gate with retained Ubuntu-24/bubblewrap namespace-startup evidence. #355 is the focused runner-capability repair and remains **UNKNOWN / pending** until its exact-head Qualification-v1 run completes. If #355 is accepted into `testing/qualification-v1`, require a new exact-head qualification on the resulting #152 head, preserve the `6cede252...` FAIL, then require explicit protected review/attestation before merge.
