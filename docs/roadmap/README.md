...
| M6.2 autonomous discovery | One bounded formal-admission PASS; follow-up evidence-use FAILs | #257/007J formally admitted one genuinely absent MeasurementGap; #259/#262/#263 still fail to use resolved evidence coherently |
...
## Autonomous discovery — #244 through #263
...
The important current gain is bounded, not general: **#257 proves one autonomous missing-measurement proposal can cross formal admission under the exact experiment contract.** The later cells show that closing the gap and supplying/resolving evidence does not yet make the Scientist reliably reason from those values.
...
3. **Evidence contracts:** make `EvidenceSnapshot` and `MeasurementGap` deterministic production contracts with canonical identity, explicit measured-vs-missing semantics, provenance, and defensive serialization.
4. **Evidence-use semantics:** after a metric is resolved, require the Scientist to incorporate the exact value into the next proposal rather than requesting it again. Repeated resolved requests should terminate/stagnate deterministically.
5. **Scientist boundary:** keep the Scientist analysis-only — no Git/write/integration/promotion authority and no ability to redefine protected invariants.
6. **HypothesisVerifier:** encode #232, #250–#255, and #259/#262/#263 rejection cases. A MeasurementGap must name a genuinely absent required measurement; an available bad value is not a missing measurement; causal/acceptance claims must remain falsifiable and evidence-grounded.
7. **Experiment ledger:** retain hypothesis/proposal identity, evidence identity, code/revision identity, model/provider identity, checks, review, receipts, outcomes, apparatus failures, and negative cells.
8. **Champion/challenger:** compare accepted candidate changes against an exact retained baseline with frozen metrics and reject regressions in protected invariants.
9. **Repeat autonomous discovery:** require repeated retained verifier-accepted proposals and complete improvement cycles before making reliability claims. One formal-admission PASS is insufficient for a general claim.
...
6. failures, apparatus defects, and regressions are retained rather than filtered from the dataset;
...
#257 satisfies only a bounded proposal-admission slice of this larger chain. Until the full criteria repeat successfully, autonomous discovery reliability and general recursive self-improvement remain **UNKNOWN / not established**.