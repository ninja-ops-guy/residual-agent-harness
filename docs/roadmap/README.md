...
3. **Resolve the onboarding/Inspector browser failure.** PR #89 head `2a4ce5dc...` has eight observed workflows **PASS**, including Inspector/onboarding qualification and Factory runtime evidence, but Pages/WebVM run `35112465799` is **FAIL** during generated desktop browser proof after earlier guest/demo/reload/audit stages passed. Preserve failure artifact `10453083802` (ZIP SHA-256 `8128c1c5d7a676cc486e0e9ab6ac9af718a6cb8aca0fadca963cdb8842198988`) and diagnose without weakening the browser gate.
...
8. **Quantify WebVM reliability.** Issues #120 and #126 remain open. The historical corruption root cause remains `UNKNOWN`, and one first-attempt release PASS does not establish a production recurrence rate. Retain every failed attempt and run a defined repeated-run campaign.
