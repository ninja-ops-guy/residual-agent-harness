...
8. **Continue the WebVM reliability diagnosis below Mission Control.** Issues #120 and #126 remain open. PR #133 now reproduces the `_PyTime_t` failure at positive `time.sleep()` call 273 without Mission Control and shows that 272+2 and 200+200 sleeps split across fresh guest CPython processes **PASS**, supporting a process-local timer/wait boundary that resets or is avoided by process replacement. The lower-level mechanism and relationship to the historical corruption family remain **UNKNOWN**. Preserve first-attempt evidence, compare alternate wait primitives/resource state and sequential fresh-process resets, then run a defined repeated-run campaign; one release PASS does not establish a production recurrence rate.
