...
| Mission Control/WebVM lifecycle | PR #136 integrated on `3a41dc1e...`; first post-merge generated + published desktop/narrow acceptance **PASS** on attempt 1 in run `35101404981`. Long-run reliability and historical corruption-family root cause remain open. PR #142 is a green candidate repair for the newly isolated positive Python timed-wait boundary, not accepted-main evidence. |
...
Several integration candidates are based directly on `main@3a41dc1e...`. Their CI is candidate-specific; none of it becomes accepted-main evidence merely because the branch is green.

The current sequence is:

1. **Review the WebVM timed-wait runtime repair.** PR #133 now shows a process-local CPython positive-duration timed-wait boundary affecting at least `time.sleep()` and `select.select()` at call 273. Direct libc `clock_nanosleep`/`nanosleep` wrappers pass beyond that boundary; raw legacy i386 `clock_nanosleep` passes; raw time64 probes are **UNSUPPORTED** with `ENOSYS`. Exact causality remains `UNKNOWN`. PR #142 head `22e00ff8...` replaces positive Python waits in the two long-lived browser paths with a fail-closed libc `nanosleep` wrapper and has all eight observed exact-head workflows **PASS**, including Pages/WebVM `35128166211` and Browser VM Demo CI `35128166368`. No submitted independent review is recorded. Require genuinely independent exact-head acceptance before integration, then require the exact merged revision's first Pages release attempt and published desktop+narrow acceptance. Do not call this long-run reliability proof.
2. **Resolve the protected M4 test-race gate.** PR #139 head `2d885527...` isolates the `/proc/<pid>/status` observation-race repair in protected `tests/test_factory_m4_safety.py`. Capable-runner M4 run `35111486331` is **PASS**, while Factory ownership and dependent workflows **FAIL closed** because the protected-byte baseline still pins the previous blob. Obtain genuinely independent exact-head review first; if accepted, deliberately advance the ownership baseline and requalify. Do not change the pin merely to make CI green.
3. **Requalify the real-provider adapter lane after #139.** PR #134 remains held after its first exact-head Command Station failure hit the protected test race. After the protected repair is accepted/integrated, refresh/requalify #134 and require independent adapter review. Keep its separate mailbox/runtime visibility failure distinct from protocol-format failures.
4. **Review the real-provider envelope-clarity candidate without broadening its claim.** Fresh real-device evidence on deployed `main@3a41dc1e...` reached a connected provider and ready guest, but the first `openai/gpt-5-nano` call failed as `provider_protocol_invalid` before verification; the bounded retry then failed separately as `provider_exception`. Draft PR #143 head `34e7e3f4...` addresses only the first class by clarifying the nested `updates.build` worker envelope. All eight observed exact-head workflows are **PASS**, including Browser VM Demo CI `35128492875` and Pages/WebVM `35128492874`, but no submitted independent review exists and no fresh post-deploy real-device success exists. Do not infer provider quality or successful inference from green CI.
5. **Review the focused browser-acceptance synchronization repair before requalifying #89.** PR #140 head `36c59627...` changes only the browser acceptance harness. All eight observed exact-head workflows are **PASS**, including Pages/WebVM `35122316061` and Browser VM Demo CI `35122316013`; no submitted review is recorded. The retained PR #89 Pages/WebVM failure in run `35112465799` remains authoritative and is not rewritten by #140. Require genuinely independent exact-head review before integration, then refresh/requalify #89 rather than treating #140's green CI as qualification of #89 itself.
6. **Obtain independent acceptance for the current-main runtime/DSM candidate.** PR #118 head `e0f042c6...` has all seven observed exact-head workflows **PASS**, including Pages/WebVM. Retained first-attempt failures remain in the record. Cross-process/multi-host serialization, production process wiring, host-loss recovery, release/recovery and elapsed soak remain separate non-claims.
7. **Obtain independent acceptance for release preparation.** PR #115 head `f1862e15...` has all seven observed exact-head workflows **PASS**, including Release preparation procedures. Its blank-VM/recovery fixtures and two-simulated-day rehearsal remain procedure/simulation evidence only, not true bare-OS, production HTTPS, actual host-loss or elapsed-soak qualification.
8. **Obtain independent acceptance for core soak-state persistence.** PR #131 head `1c416970...` has all seven observed exact-head workflows **PASS**, including Pages/WebVM. Its scope remains local resumable-state persistence hardening; it does not create elapsed 24h/72h/30-day soak evidence.
9. **Obtain independent acceptance for economics/observability.** PR #93 head `17189262...` has all eight applicable workflows **PASS**, including Economics and observability qualification and Pages/WebVM. Retained/generated results remain development-fixture evidence; they do not establish live SLOs, real-model economics, production reliability, confirmatory research results or release readiness.
10. **Continue the WebVM root-cause investigation separately from the bounded repair.** Keep issues #120 and #126 open. Inspect CPython/glibc timed-wait ABI/fallback behavior, repeat fresh-process resets across several processes and compare alternate/minimal WebVM runtimes. PR #142 can remove the known vulnerable wait use from long-lived browser paths without proving the lower mechanism or its relationship to the historical `_sha512`, impossible-constructor and `munmap_chunk()` family.
11. **Execute broader release/recovery qualification.** Exercise blank-environment setup, actual recovery and retained-evidence procedures without converting rehearsal or simulation evidence into release PASS.
12. **Freeze the live evaluation protocol and selected evidence path.** Lock workload, task mapping, run identity, model/configuration, verifier policy/boundary, scheduler/topology evidence, metrics and analysis before observing confirmatory model results.
13. **Run R0–R5 with one fixed live model.** Measure raw correctness, acceptance coverage, accepted correctness, AER/ASSR, verifier false acceptance/rejection/`UNKNOWN`, cost, latency and throughput.
14. **Run model-degradation + heterogeneous-routing studies.** Test whether cheaper/weaker workers can contribute safely under the same acceptance boundary.
15. **Run live fault campaigns and staged soak tests.** 24-hour → 72-hour → 30-day only after shorter gates are clean.
16. **Promote paper claims only from retained evidence.** Negative, `UNKNOWN`, rejected and failed runs stay in the record.
