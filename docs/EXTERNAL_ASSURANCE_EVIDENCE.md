...
The factory runtime on `main` now provides bounded worker contracts, isolated worktrees, swarm identities, runtime journaling, and kernel-enforced execution controls. This external provider benchmark remains intentionally **direct-engine-only** until the assurance strategy layer is explicitly adapted to that factory runtime and its swarm outcomes are incorporated into the same preregistered evidence protocol. Therefore this report does not yet claim live swarm evidence.
